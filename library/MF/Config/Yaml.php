...
class MF_Config_Yaml extends Zend_Config {
  /**
...
//      throw new Zend_Config_Exception('Syck extension is not loaded');
    ...
    $ymlArray = array();
  	if(is_callable('yaml_parse_file'))                       // ##### PECL
	{
		$ymlArray = yaml_parse_file($filename);
		restore_error_handler();
	}
	elseif(is_callable('syck_load'))                         // ##### Python extension
	{
	...
	else                                                     // ##### Symfony Parser
	{
		...
		$ymlArray = $parser->parse(file_get_contents($filename));
		restore_error_handler();
	}
