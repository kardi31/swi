 ...
 *
 */
class MF_Paginator_Adapter_Doctrine implements Zend_Paginator_Adapter_Interface
{
	/**
	...
	public function __construct($query, $hydrationMode = null, $options = array())
	{
		if ( is_string($query) ) {
			...
			throw new Exception("Given query is not instance of Doctrine_Query");
		}
		$this->_query = $query;
		...
		if ( !empty($options['countQuery']) ) {
			if ( is_string($options['countQuery']) ) {
				...
				require_once 'SmartL/Zend/Paginator/Adapter/Doctrine/Exception.php';
				throw new SmartL_Zend_Paginator_Adapter_Doctrine_Exception("Given count-query is not instance of Doctrine_Query");
			}
	...
	{
        return $this->getCountQuery()->count();
