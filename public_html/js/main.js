...
//					if($(this).hasClass('bpat1')) {$('#mainContent').css('background', 'url(images/patterns/body/cream_dust.png)');}
//					if($(this).hasClass('bpat2')) {$('#mainContent').css('background', 'url(images/patterns/body/dust.png)');}
//					if($(this).hasClass('bpat3')) {$('#mainContent').css('background', 'url(images/patterns/body/grey.png)');}
//					if($(this).hasClass('bpat4')) {$('#mainContent').css('background', 'url(images/patterns/body/subtle_dots.png)');}					
//				});
//
...
//        if($this.attr('href') == path) {
        if(path.match($this.attr('href'))) {
            ...
            var parents = $this.parentsUntil("#mainnav > ul");
            parents.each(function(index) {
                var sub = parents[index];
                ulElem = $(sub).closest('ul');
                if(ulElem.hasClass('sub')) {
                    //its a part of sub menu need to expand this menu
                    aElem = ulElem.prev('a.hasUl').addClass('drop');
                    ulElem.addClass('expand');
	...
	mainNavLink.click(function(event) {
		...
		if($this.hasClass('hasUl')) {
			...
			if($this.hasClass('drop')) {
				$(this).siblings('ul.sub').slideUp(50, 'jswing').siblings().removeClass('drop');
			} else {
				$(this).siblings('ul.sub').slideDown(50, 'jswing').siblings().addClass('drop');
			}			
		} else {
	...
	mainNavSubLink.click(function(event) {
		...
		if($this.hasClass('hasUl')) {
			event.preventDefault();
			if($this.hasClass('drop')) {
				$(this).siblings('ul.sub').slideUp(50).siblings().removeClass('drop');
			} else {
				$(this).siblings('ul.sub').slideDown(50).siblings().addClass('drop');
			}			
		} else {
			//has no ul so store a cookie for change class.
			//$.cookie("newCurrentMenu",$this.attr('href') ,{expires: null});
	...
	$(function () {
		...
		for ( var i = 0, j = pages.length; i < j; i++ ) {
		    ...
		    if($.cookie("currentPage") == pages[i]) {
				...
				if($('#mainContent-two').length) {
					$('#mainContent-two').css('margin-left', '0');
				}
		    }
	...
	$( '.collapseBtn' ).bind( 'click', function(){
		...
		if ($this.hasClass('leftbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				$('#mainContent').css('margin-left', '213'+'px');
				$('#mainContent-two').css('margin-left', '213'+'px');
		...
		if ($this.hasClass('rightbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				if($('#mainContent-one').length) {
					$('#mainContent-one').css('margin-right', '212'+'px');
				}
				if($('#mainContent-two').length) {
					$('#mainContent-two').css({'margin-right' : '212' + 'px'});
				}			
				/*if($('#sidebar').length) {
					$('#sidebar').css({'left' : '0', 'margin-left' : '0'});
				}*/
				...
				if($('#mainContent-one').length) {
					$('#mainContent-one').css({'margin-left': '0', 'margin-right' : '0'});
				}
				if($('#mainContent-two').length) {
					$('#mainContent-two').css({'margin-right' : '0'});
				}	
