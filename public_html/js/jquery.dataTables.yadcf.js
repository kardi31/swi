	...
	function rangeClear(table_selector_jq_friendly, event) {
		...
		var oTable = oTables[table_selector_jq_friendly],
			...
			column_number;
                ...
                // modyfikacja do CMS Varts 
                // dodane 3x parent zamiast 1x parent
                // Tomasz Kardas
		column_number = parseInt($(event.target).parent().parent().parent().attr("id").replace('yadcf-filter-wrapper-' + table_selector_jq_friendly + '-', ''), 10);
		...
		if (!oTable.fnSettings().oLoadedState) {
			...
			$("#yadcf-filter-" + table_selector_jq_friendly + "-from-date-" + column_number).val('');
			$("#yadcf-filter-" + table_selector_jq_friendly + "-to-date-" + column_number).val('');
		if (oTable.fnSettings().oFeatures.bStateSave === true) {
			if (oTable.fnSettings().oLoadedState.yadcfState !== undefined && oTable.fnSettings().oLoadedState.yadcfState[table_selector_jq_friendly] !== undefined) {
